...
const string strMessageMagic = "InkWayCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight >= 1 && pindexBest->nHeight <= 201)
    {
        nSubsidy = 50000 * COIN;
    }
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nHeight < POW_BLACKOUT_END_BLOCK && nHeight > POW_BLACKOUT_BEGIN_BLOCK)
...
// inkwaycoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "InkWayCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Sept 21, 2014 - The InkWayCoin is finally here";
        CTransaction txNew;
        txNew.nTime = 1411319560;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 922183 << CBigNum(9999) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xf7c16db6db294026424bd5c4d75522b282a9e41bf797832425fca00129dfe3ae"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe8, 0xc0, 0xa2, 0xf3 };
